...
Begin:
	auto ratio = Float((now - BeginTime).count()) / Float(Duration.count());
	// Animation has not begun. $BeginTime is not reached by $now.
	if (ratio < 0) return { Float(0), AnimationState::Ready };
	...
	if (ShouldReverse)
	{
		...
		if (RepeatTimes == 0) return { Float(0), AnimationState::Finished };
		// Backward stage.
		if (ratio < 2) return { Float(2) - ratio, AnimationState::Backward };
