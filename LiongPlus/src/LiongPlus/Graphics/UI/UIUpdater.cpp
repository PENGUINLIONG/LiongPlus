...
#include "LiongPlus/Graphics/UI/_UIAnimationUpdater.hpp"
#include "LiongPlus/Graphics/UI/_UIActionUpdater.hpp"
#include "LiongPlus/Threading/Timer.hpp"
...
namespace UIUpdater
{
	namespace details
	{
		...
		const microseconds frameInterval = 16'000us;

		struct UpdaterData
		{
			shared_ptr<Timer<microseconds>> _Timer;
		...
		void doUpdate(const shared_ptr<Timer<microseconds>>& sender,
			const TimeElapsedEventArgs<microseconds>& args)
		{
	...
	void StartUIUpdater()
	{
		...
		{
			data->_Timer = std::make_shared<Threading::Timer<microseconds>>();
	...
	void StopUIUpdater()
	{
		details::getUpdaterDataOfCurrentThread().get()->_Timer->Stop();
	}
	...
	void _Unregister(const I<UIActionHost>& target)
	{
		...
		auto it = std::find(data->_Targets.begin(),
			data->_Targets.end(), target);
...
UIUpdaterGuard::UIUpdaterGuard()
{
	UIUpdater::StartUIUpdater();
}
UIUpdaterGuard::~UIUpdaterGuard()
{
	UIUpdater::StopUIUpdater();
}
