...
public:
	...
	Array(const size_t size) noexcept :
		_Size(size),
		_Ptr(size == 0 ? nullptr : new T[size]),
		_ShouldDelete(size != 0)
	{}
	template<typename TIter>
	Array(const TIter& beg, const TIter& end) noexcept :
		_Size(std::distance(beg, end)),
		_Ptr(size == 0 ? nullptr : new T[_Size]),
		_ShouldDelete(size != 0)
	{
		std::copy(beg, end, _Ptr);
	}
	Array(T* pointer, size_t size,
		bool shouldDelete = true) noexcept :
		_Size(size),
		_Ptr(pointer),
	...
	Array(const Array<T>& instance) noexcept :
		...
		_Ptr(instance._Size == 0 ? nullptr : new T[instance._Size]),
		_ShouldDelete(instance._Size != 0)
	{
		std::copy(instance.begin(), instance.end(), _Ptr);
	}
	Array(Array<T>&& instance) noexcept :
		_Size(std::exchange(instance._Size, 0)),
		_Ptr(std::exchange(instance._Ptr, nullptr)),
		_ShouldDelete(std::exchange(instance._ShouldDelete, false))
	{
	}
	...
	{
		if (std::addressof(instance) == this) return *this;
