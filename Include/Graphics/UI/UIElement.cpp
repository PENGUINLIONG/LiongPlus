...
UIElement::TransformProperty = UIProperty::Make<Matrix4x4>(
	...
	[]() { Matrix4x4* m = new Matrix4x4; *m = Matrix4x4::Identity(); return m; },
...
const UIProperty
UIElement::DispatcherProperty = UIProperty::Make<std::shared_ptr<Dispatcher>>(
	"Dispatcher", UIActionTrigger::None,
	[](const std::shared_ptr<Dispatcher>& dispatcher) { return true; },
	[]() { return new std::shared_ptr<Dispatcher>(Dispatcher::Current()); },
	[](void* d) { delete reinterpret_cast<std::shared_ptr<Dispatcher>*>(d); }).Register<UIElement>();
const UIProperty
...
const std::shared_ptr<Dispatcher>& UIElement::GetDispatcher() const
{
	return GetProperty<std::shared_ptr<Dispatcher>>(DispatcherProperty);
}
