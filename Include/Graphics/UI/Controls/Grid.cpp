...
void Grid::SetColumn(IUIElement& uie, unsigned int col)
{
...
void Grid::SetRow(IUIElement& uie, unsigned int row)
{
...
Size Grid::Layout(const Size& containerSize)
{
	...
	for (const auto& child : GetProperty<UIElementCollection>(ChildrenProperty))
	{
		...
		if (colAutoAlloc[col] || rowAutoAlloc[row])
		{
			...
			colInfo.SizeRemain -= sizeAfter.Width;
			rowInfo.SizeRemain -= sizeAfter.Height;
