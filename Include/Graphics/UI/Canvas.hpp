...
public:
	...
	virtual void DrawArc(double radius, double fromDeg, double angleDegree, const Point& center, bool enclose) = 0;
	virtual void DrawBezier(const Array<Point>& points) = 0;
	virtual void DrawCircle(double radius, const Point& center) = 0;
	virtual void DrawFan(double radius, double fromDeg, double angleDegree, const Point& center) = 0;
	 ...
	 *     FLOATS.
	 */
	virtual void DrawDirectly(const float* points, size_t count) = 0;
};
...
public:
	RenderEventArgs(const ICanvas& canvas);
	RenderEventArgs(ICanvas&& canvas);

	virtual ICanvas Canvas();
};
